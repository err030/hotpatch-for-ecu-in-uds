...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default="01_software_level/results/hardware_fuzz")
    ...
    with log_path.open("w", encoding="utf-8", buffering=1) as log:
        for run_index, seed in enumerate(seeds, start=1):
            for patch_state in ("before", "after"):
                ...
                time.sleep(1.0)
                # Keep artifact names stable and date-free. The exact seed remains in
                # the generated CSV/log metadata for reproducibility.
                tag = f"run{run_index}_{patch_state}"
                command = [
                    sys.executable,
                    str(script),
                    "--interface",
